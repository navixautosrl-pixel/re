...
{else}
    ...
    <div class="bg-seccolorstyle bg-white mergecolor mt-50 p-50 br-12">
        <p class="text-center h4 mb-5">{$LANG.upgradecurrentconfig}:<br/><strong>{$groupname} - {$productname}</strong>{if $domain} ({$domain}){/if}</p>
        <p class="text-center">{$LANG.upgradechoosepackage}</p>
